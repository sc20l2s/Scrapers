...
# Base URL for El Mundo Archive
ARCHIVE_URL = "https://www.elmundo.es/elmundo/hemeroteca/{year}/{month}/{day}/{edition}/index.html"
EDITIONS = ["m", "t", "n"]  # Morning, afternoon, and night editions
...
def valid_filename(filename, default="elmundo_archive.txt"):
...
async def fetch_page(session, url, max_retries=5, base_delay=3):
    ...
    while retries < max_retries:
        ...
        try:
            ...
            return None
    print(f"Failed after {max_retries} retries: {url}")  # Basically, El Mundo told us to fuck off
    return None
...
async def scrape_archive(session, year, month, day, keywords):
    async with semaphore:
        ...
        for edition in EDITIONS:
            ...
            if not html:
                continue  # Skip if page not found or failed

            soup = BeautifulSoup(html, "html.parser")

            # Find all article containers
            for article in soup.find_all("article", class_="ue-c-cover-content"):
                link_tag = article.find("a", class_="ue-c-cover-content__link", href=True)
                if link_tag:
                    headline_tag = link_tag.find("h2", class_="ue-c-cover-content__headline")
                    if headline_tag:
                        ...
                        if not link.startswith("http"):
                            link = urljoin("https://www.elmundo.es", link)
...
async def main():
    ...
    with open(filename, "w", encoding="utf-8") as f:
        f.write(f"--== Extracted {len(all_articles)} articles from El Mundo ({year}) ==--\n\n")
